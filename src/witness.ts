...
import { computeCommitment, createDummyNoteWithTokenId, computeNullifier } from "./notes.js";
import {
  type OwnedNote,
  type Note,
  type MerkleTree,
...
async function getEddsa(): Promise<Awaited<ReturnType<typeof buildEddsa>>> {
  if (!eddsaInstance) {
 ...
 */
export async function buildTransactWitness(
  inputNotes: OwnedNote[],
  outputNotes: Note[],
  merkleTree: MerkleTree,
  keys: ShadeKeys,
  ...
  if (inputSum !== outputSum) {
    throw new Error(
      `Value mismatch: inputs sum to ${inputSum} but outputs sum to ${outputSum}`,
    );
  }
  ...
  const commitmentsOut: bigint[] = paddedOutputs.map((n) => n.commitment);

  // Compute Merkle proofs
  const pathElements: bigint[][] = [];
  const leavesIndices: number[] = [];
