...
import { SNARK_FIELD, } from "./types.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
function deriveSymmetricKey(sharedPoint) {
...
function serialiseNote(note) {
    return concatBytes(bigIntToBytes32(note.npk), bigIntToBytes32(note.token), bigIntToBytes32(note.value), bigIntToBytes32(note.random));
}
/**
 * Deserialise a 128-byte plaintext back into partial note fields.
 */
function deserialiseNote(plaintext) {
 ...
 */
export async function encryptNote(note, senderRandom, receiverViewingPubKey) {
    const babyjub = await getBabyjub();
    // Decode receiver viewing public key (64 bytes → two field elements)
    if (receiverViewingPubKey.length !== 64) {
        throw new Error(`receiverViewingPubKey must be 64 bytes, got ${receiverViewingPubKey.length}`);
    }
    ...
    const nonceSource = sha256(concatBytes(bigIntToBytes32(senderRandom), bigIntToBytes32(ephemeralPubKey[0])));
 ...
 */
export async function decryptNote(ciphertext, viewingKey) {
    const babyjub = await getBabyjub();
    try {
 ...
 */
export async function viewingPublicKey(viewingKey) {
    const babyjub = await getBabyjub();
    let scalar = 0n;
    for (const b of viewingKey) {
 ...
 */
export async function decryptShieldCiphertext(_ct, _viewingKey) {
 ...
 */
export async function decryptTransactCiphertext(ct, viewingKey) {
