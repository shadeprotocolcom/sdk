...
import { VIEWING_KEY_DOMAIN, } from "./types.js";
// ---------------------------------------------------------------------------
 ...
 */
export function fieldToBigInt(poseidon, element) {
 ...
 */
export async function deriveShadeKeys(signature) {
    ...
    const viewingKey = sha256(concatBytes(sigBytes, utf8ToBytes(VIEWING_KEY_DOMAIN)));
    // --- Nullifying key ---
 ...
 */
export async function generateNotePublicKey(masterPublicKey, random) {
    const poseidon = await getPoseidon();
